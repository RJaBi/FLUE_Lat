module FLUE_gluonProp
  ...
  !interface qhat
  !   module procedure qhat_1
  !   module procedure qhat_a
  !   module procedure qhat_a4
  !end interface qhat
...
contains
  subroutine say_hello
    print *, "Hello, gluonProp!"
  end subroutine say_hello


  pure function qhat(coord, shape)
    integer, dimension(4), intent(in)  :: coord, shape
    real(kind=WP), dimension(4)        :: qhat
    qhat = ( 2.0_WP * PI) * real(coord, kind=WP) / real(shape, kind=WP)
  end function qhat

  pure function qhat_a(coord, shape, a)
    ! This isnt correct but left as example of itnerface
    integer, dimension(4), intent(in)  :: coord, shape
    real(kind=WP),         intent(in)  :: a
    real(kind=WP), dimension(4)        :: qhat_a
    qhat_a = ( 2.0_WP * PI / a ) * real(coord, kind=WP) / real(shape, kind=WP)
  end function qhat_a


  

  pure subroutine calc_mom_space_scalarD(U, D, D4, mu_start, xi)
    complex(kind=WC), dimension(:,:,:,:,:,:,:), intent(in)  :: U ! NT, NS, NS, NS, ND, NC, NC
    integer,                                    intent(in)  :: mu_start
    real(kind=WP),                              intent(in)  :: xi
    complex(kind=WC), dimension(:,:,:,:),       intent(out) :: D, D4
    !
    ...
    do concurrent(tt=0: U_shape(1)/2, qx=0: U_shape(2)/2, qy=0: U_shape(3)/2, qz=0: U_shape(4)/2, mu=mu_start: U_shape(5))
       ...
       ! (i/2) * exp(-pi Q/ N) * A_
       A_pos = (0.0_WP, 0.5_WP) * exp(-(0.0_WP, 1.0_WP) * PI * real(coord(mu), kind=WP) / real(U_shape(mu), kind=WP)) * A_pos
       A_neg = (0.0_WP, 0.5_WP) * exp((0.0_WP, 1.0_WP) * PI * real(coord(mu), kind=WP) / real(U_shape(mu), kind=WP)) * A_neg
    ...
    do concurrent(tt=0: U_shape(1)/2, qx=0: U_shape(2)/2, qy=0: U_shape(3)/2, qz=0: U_shape(4)/2)
       ...
       if (mu_start == 1 .and. qx + qy + qx < 1) then
          ...
          prefac = 2.0_WP / (8.0_WP * real(U_shape(1), kind=WP) * real(U_shape(2), kind=WP) * real(U_shape(3), kind=WP) * real(U_shape(4), kind=WP) * real(U_shape(5), kind=WP))
       else if (mu_start == 0 .and. tt + qx + qy + qz + qx < 1) then
          ! (nc*nc -1) * nd *NT*NS*NS*NS
          prefac = 2.0_WP / (8.0_WP * real(U_shape(1), kind=WP) * real(U_shape(2), kind=WP) * real(U_shape(3), kind=WP) * real(U_shape(4), kind=WP) * real(U_shape(5), kind=WP))
       else
          ! (nc*nc -1) * (nd-1) *NT*NS*NS*NS
          prefac = 2.0_WP / (8.0_WP * real(U_shape(1), kind=WP) * real(U_shape(2), kind=WP) * real(U_shape(3), kind=WP) * real(U_shape(4), kind=WP) * 3.0_WP)
       end if
       ! apply
