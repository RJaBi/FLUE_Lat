...
contains
  ...
  subroutine cone_cut(NQ, radius, Q, D, D4, NT, NS, angleIN, xiIN, IRCutIN, IRRadiusIN, QOut, Dout, D4Out, qcount)
    ...
    real(kind=WP), parameter, dimension(4) :: BCD_norm = (/0.5, 0.5, 0.5, 0.5/)
    ...
    do qq =1, NQ
       ...
       !write(*,*) qq, q(qq, :), qhat
       qhat = q(qq, :)
       q_norm = sqrt(sum(real(qhat,kind=WP)**2.0))
       ...
       if (q_norm .le. IRCut) then
          cone_mask(qq,:) = r .le. IRRadius
       else
          cone_mask(qq,:) = r .le. IRRadius .and. theta < angle
       endif
       !if (qq == -1) then
       !   write(*,*) 'r', r
       !   write(*,*) 'theta', theta
