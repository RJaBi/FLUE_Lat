            ...
            do concurrent(nnx=1:nx, nny=1:ny, nnz=1:nz, nnt=1:nt) &
               ...
               function Loop5MuNuCoord(data, NT, NX, NY, NZ, mu, nu) result(U_xd)
                  ...
                  do concurrent(nnx=1:nx, nny=1:ny, nnz=1:nz, nnt=1:nt) &
                     ...
                     local(coordBase, clovLeaf, trcnsub, thisU, c12, c13, c23, workU) shared(U_xd)
#else
                     do concurrent(nnx=1:nx, nny=1:ny, nnz=1:nz, nnt=1:nt)
#endif
                        ...
                        if (ALL(coordBase == (/1, 1, 1, 1/))) then
                           write (*, *) mu, nu, '1x1', 1, clovLeaf(1, 1)
                        end if
                        ...
                        thisU = thisU + clovLeaf
                        if (ALL(coordBase == (/1, 1, 1, 1/))) then
                           write (*, *) mu, nu, '1x1', 2, clovLeaf(1, 1)
                        end if
                        ! 3
                        clovLeaf = genericPath(data, coordBase, path1x1(3, :))
                        thisU = thisU + clovLeaf
                        if (ALL(coordBase == (/1, 1, 1, 1/))) then
                           write (*, *) mu, nu, '1x1', 3, clovLeaf(1, 1)
                        end if
                        ! 4
                        clovLeaf = genericPath(data, coordBase, path1x1(4, :))
                        if (ALL(coordBase == (/1, 1, 1, 1/))) then
                           write (*, *) mu, nu, '1x1', 4, clovLeaf(1, 1)
                        end if
                        thisU = thisU + clovLeaf
                        thisU = thisU * k1
                     ...
                     function plaquetteMuNuCoord(data, NT, NX, NY, NZ, mu, nu) result(U_xd)
                        ...
                        do concurrent(nnx=1:nx, nny=1:ny, nnz=1:nz, nnt=1:nt) &
                           ...
                           function magnetic(data, NT, NX, NY, NZ) result(B)
                              ...
                              ! i.e. 32 is conjugate of 23
                              ! not relevant for RealTrace
                              !BField(:, :, :, :, :, :, 1) = cloverLoopMuNuCoord(data, NT, NX, NY, NZ, 3, 2)
                              !BField(:, :, :, :, :, :, 2) = cloverLoopMuNuCoord(data, NT, NX, NY, NZ, 2, 4)
                              !BField(:, :, :, :, :, :, 3) = cloverLoopMuNuCoord(data, NT, NX, NY, NZ, 4, 3)
